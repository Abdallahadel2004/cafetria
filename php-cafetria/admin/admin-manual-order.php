...
<body>
            ...
            <div class="topbar-actions">
                <div class="search-box" style="position:relative">
                    <span class="material-symbols-outlined"
                        style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--on-surface-variant); font-size:20px">search</span>
                    <input type="text" id="prodSearch" class="form-control" placeholder="Search products..."
                ...
                <div class="cart-card section-card">
                    ...
                    <div style="padding:20px">
                        ...
                        <div class="alert alert-success"
                            style="margin-bottom:16px; padding:10px; border-radius:8px; background:#e6f4ea; color:#1e7e34; font-size:14px">
                            <?= $formSuccess ?></div>
                        <?php endif; ?>
                        <?php if ($formError): ?>
                        <div class="alert alert-danger"
                            style="margin-bottom:16px; padding:10px; border-radius:8px; background:#fce8e6; color:#d93025; font-size:14px">
                            <?= $formError ?></div>
                        <?php endif; ?>

                        <div id="cartItems" class="cart-list">
                            <div
                                style="text-align:center; color:var(--on-surface-variant); padding:20px; font-size:14px">
                                Your cart is empty</div>
                        </div>

                        <div
                            style="display:flex; justify-content:space-between; margin-bottom:20px; padding-top:10px; border-top:2px solid var(--outline-variant)">
                            <span style="font-weight:700">Total</span>
                            <span id="cartTotal" style="font-weight:800; color:var(--primary); font-size:1.1rem">0.00
                                EGP</span>
                        </div>

                        <!-- Cart-level error (no items) -->
                        <div class="field-error" id="err-cart" style="margin-bottom:10px; text-align:center;">
                        ...
                        <form method="POST" id="orderForm" novalidate>
                            ...
                            <div class="form-group" style="margin-bottom:16px">
                                ...
                                <select name="user_id" id="f-user" class="form-control">
                                    ...
                                    <option value="<?= $u['id'] ?>"
                                        data-room="<?= htmlspecialchars($u['room'] ?? '') ?>">
                                        <?= htmlspecialchars($u['name']) ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="field-error" id="err-user">Please select a customer.</div>
                            </div>

                            <div class="form-group" style="margin-bottom:16px">
                                <label class="form-label">Room</label>
                                <input type="text" name="room" id="f-room" class="form-control"
                                    placeholder="Room Number">
                                <div class="field-error" id="err-room">Room number is required.</div>
                            </div>
                ...
                <div class="products-col">
                    ...
                    <div style="margin-bottom:32px">
                        <h3 class="category-title"
                            style="margin-bottom:16px; font-size:1rem; color:var(--on-surface-variant); text-transform:uppercase; letter-spacing:1px">
                            <?= htmlspecialchars($catName) ?>
                        </h3>
                        <div class="prod-grid">
                            <?php foreach ($prods as $p): ?>
                            <div class="prod-card" id="pc-<?= $p['id'] ?>" data-id="<?= $p['id'] ?>"
                                ...
                                <div class="prod-img" style="display:flex; align-items:center; justify-content:center">
                                    <span class="material-symbols-outlined"
                                        style="font-size:32px; color:var(--on-surface-variant)">coffee</span>
                                </div>
                                <?php endif; ?>
                                ...
                                <div style="color:var(--primary); font-weight:700; margin-top:4px">
                                    <?= number_format($p['price'], 2) ?> EGP</div>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
    ...
    function addToCart(el) {
        ...
        // clear cart error as soon as something is added
        clearError('err-cart', 'f-user');
        renderCart();
    }
    ...
    document.getElementById('f-user').addEventListener('change', function() {
        ...
        clearError('err-user', 'f-user');
    });

    document.getElementById('f-room').addEventListener('input', function() {
        if (this.value.trim() !== '') clearError('err-room', 'f-room');
    });
    ...
    document.getElementById('orderForm').addEventListener('submit', function(e) {
        ...
        if (Object.keys(cart).length === 0) {
            showError('err-cart', null);
            valid = false;
        }

        // 2. User must be selected
        const userEl = document.getElementById('f-user');
        if (!userEl.value) {
            showError('err-user', 'f-user');
            valid = false;
        }

        // 3. Room must be filled and valid
        const roomEl = document.getElementById('f-room');
        const roomVal = roomEl.value.trim();
        if (roomVal === '') {
            document.getElementById('err-room').textContent = 'Room number is required.';
            showError('err-room', 'f-room');
            valid = false;
        } else if (!/^[\w\-]{1,20}$/.test(roomVal)) {
            document.getElementById('err-room').textContent = 'Room contains invalid characters.';
            showError('err-room', 'f-room');
            valid = false;
        }
