...
<body>
        ...
        <div class="page-content">
            <div class="section-card">
                <div class="section-header">
                    <div class="section-title"><span class="material-symbols-outlined">restaurant_menu</span>Product
                        Catalog</div>
                            ...
                            <tr id="product-row-<?= (int)$p['id'] ?>"
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:10px">
                                        <span
                                            style="font-size:1.5rem"><?= htmlspecialchars($p['emoji'] ?: '🍽️') ?></span>
                                        <div>
                                ...
                                <td><span class="badge badge-category"><?= htmlspecialchars($p['category']) ?></span>
                                </td>
                                <td><span
                                        style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;font-size:0.95rem">EGP
                                        <?= number_format($p['price']) ?></span></td>
                                <td><span
                                        class="badge badge-<?= $p['status'] === 'Available' ? 'available' : 'unavailable' ?> status-badge"
                                        id="status-<?= (int)$p['id'] ?>"><?= htmlspecialchars($p['status']) ?></span>
                                </td>
                                <td><?= (int)$p['total_orders'] ?> orders</td>
                                <td>
                                    <div style="display:flex;gap:6px">
                                        <button class="btn btn-secondary btn-sm"
                                            onclick='openEditProduct(<?= json_encode(["id" => (int)$p['id'], "name" => $p['name'], "category" => $p['category'], "price" => (int)$p['price'], "status" => $p['status'], "emoji" => $p['emoji'], "desc" => $p['description']]) ?>)'><span
                                                class="material-symbols-outlined">edit</span></button>
                                        <button class="btn btn-secondary btn-sm"
                                            onclick="toggleProduct(<?= (int)$p['id'] ?>)"><span
                                                class="material-symbols-outlined"><?= $p['status'] === 'Available' ? 'visibility_off' : 'visibility' ?></span></button>
                                        <button class="btn btn-danger btn-sm"
                                            onclick="deleteProduct(<?= (int)$p['id'] ?>)"><span
    ...
    <div class="modal-overlay" id="add-product-modal">
        <div class="modal">
            ...
            <div class="modal-actions"><button class="btn btn-secondary"
                    onclick="closeModal('add-product-modal')">Cancel</button><button class="btn btn-primary"
                    onclick="submitAddProduct()">Add</button></div>
        </div>
    </div>
    <div class="modal-overlay" id="edit-product-modal">
        <div class="modal">
            <h3>Edit Product</h3>
            <div class="modal-actions"><button class="btn btn-secondary"
                    onclick="closeModal('edit-product-modal')">Cancel</button><button class="btn btn-primary"
                    onclick="submitEditProduct()">Save</button></div>
        </div>
    </div>
