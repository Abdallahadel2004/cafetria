        ...
        <div class="topbar">
            ...
            <div class="topbar-actions">
                <div class="search-wrap">
                    ...
                    <input class="search-input" type="text" id="orders-search" placeholder="Search orders..." oninput="filterOrdersTable()">
                </div>
        ...
        <div class="page-content">
                ...
                <div class="table-wrap">
                    <table id="orders-table">
                        ...
                        <tbody id="orders-table-body">
                            <?php if (empty($orders)): ?>
                            <tr>
                                <td colspan="9" style="text-align:center;padding:2rem;color:var(--on-surface-variant)">No orders found</td>
                            </tr>
                            <?php else: ?>
                            <?php foreach ($orders as $o): ?>
                            <tr data-status="<?= htmlspecialchars($o['status']) ?>" data-search="<?= strtolower(htmlspecialchars($o['customer'].' '.$o['items_summary'])) ?>">
                                <td><strong style="color:var(--primary)">#<?= htmlspecialchars($o['id']) ?></strong></td>
                                <td style="font-size:0.78rem"><?= htmlspecialchars($o['created_at']) ?></td>
                                <td><?= htmlspecialchars($o['customer']) ?></td>
                                <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= htmlspecialchars($o['items_summary']) ?></td>
                                <td><?= htmlspecialchars($o['room']) ?></td>
                                <td><?= htmlspecialchars($o['ext']) ?></td>
                                <td><strong>EGP <?= number_format($o['total']) ?></strong></td>
                                <td><span class="badge badge-<?= strtolower(htmlspecialchars($o['status'])) ?>"><?= htmlspecialchars($o['status']) ?></span></td>
                                <td>
                                    <div style="display:flex;gap:6px;align-items:center">
                                        <?php if ($o['status'] === 'Processing'): ?>
                                        <button class="btn btn-primary btn-sm" onclick="deliverOrder(<?= (int)$o['id'] ?>, this)">
            ...
            <div id="orders-card-view" style="display:none">
                <div class="orders-grid">
                    <?php foreach ($orders as $o): ?>
                    <div class="order-card" data-status="<?= htmlspecialchars($o['status']) ?>" data-search="<?= strtolower(htmlspecialchars($o['customer'].' '.$o['items_summary'])) ?>">
                        <div class="order-card-header">
                            <span class="order-number">#<?= htmlspecialchars($o['id']) ?></span>
                            <span class="badge badge-<?= strtolower(htmlspecialchars($o['status'])) ?>"><?= htmlspecialchars($o['status']) ?></span>
                        </div>
                        <div class="order-card-body">
                            <div class="order-meta"><span class="material-symbols-outlined">person</span><?= htmlspecialchars($o['customer']) ?></div>
                            <div class="order-meta"><span class="material-symbols-outlined">meeting_room</span>Room <?= htmlspecialchars($o['room']) ?> · Ext. <?= htmlspecialchars($o['ext']) ?></div>
                            <div class="order-meta"><span class="material-symbols-outlined">schedule</span><?= htmlspecialchars($o['created_at']) ?></div>
                            ...
                            <div style="display:flex;gap:8px;margin-top:0.75rem">
                                <button class="btn btn-primary btn-sm" style="flex:1" onclick="deliverOrder(<?= (int)$o['id'] ?>, this)">
